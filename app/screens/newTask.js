...
import {View, LogBox, Text, StyleSheet, Button, Alert, Switch} from 'react-native'
...
class newTask extends React.Component{
  ...
  componentDidMount(){
    ...
    const roundedOffDate = new Date(Math.floor(this.state.date / milldate) * milldate)
  ...
  return (
    <View style={styles.mainView}>
      ...
      <TextInput style={selfStyle.inputText} 
      placeholder={'Write the name of the task. (maximum length = 50)'} 
      maxLength={50} onChangeText={text => this.setState({name: text})}/>
      <View>
        <Text>                                        </Text>
        <Text style={selfStyle.date_time}> Date: {this.state.tdate+"/"+this.state.tmonth+"/"+this.state.tyear} </Text>
        <Text>                                        </Text>
      ...
      <View style={{flexDirection: 'row', paddingLeft: 5}}>
        ...
        <Switch value={this.state.isDaily} onValueChange={() => this.setState({isDaily: !this.state.is24Hour})}/>
      </View>
      {show && (
        <DateTimePicker
      ...
      <Text>                                        </Text>
      <Text>                                        </Text>
      <Button color='#841584' title={'Add Task'} disabled={this.checkButton()}
