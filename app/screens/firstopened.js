...
class firstOpened extends React.Component{
  ...
  dispatchUsername = () => {
	  if (this.state.usePassword){
    	...
    	store.dispatch(updateUser({
				...
				language: this.state.secondaryLanguage, autoLogin: this.state.autoLogin,
  ...
  if (this.state.user === 'undefined' && this.props.appCount === 0){
    return(<View style={styles.frontPageView}>
			<Text style={styles.smalltexting}>Enter Your Name To Begin</Text>
			<TextInput style = {styles.firstPageInput} value={this.state.savingUser} autoCapitalize='words'
			placeholder={'Enter your name'} maxLength={30} placeholderTextColor={'white'}
			keyboardType='default' onChangeText={(text) => this.setState({savingUser: text})}/>
		<ModalSelector data={this.languages} touchableStyle={{backgroundColor: 'orange'}} onChange={
			option => {
			this.setState({secondaryLanguage: option.name})
			}}
			keyExtractor={item => item.id} labelExtractor={item => item.language} style={{paddingBottom: 60}}
			initValue='Choose language to receive notification' cancelText='Cancel' />
		<View style={{flexDirection: 'row', paddingLeft: 5}}>
			<Text style={{paddingRight: 20}}>Would You Like To have a Password</Text>
			<Switch value={this.state.usePassword} onValueChange={() => this.setState({usePassword: !this.state.usePassword})}/>
		</View>
		<TextInput style = {styles.firstPageInput} value={this.state.password} autoCapitalize='words'
			placeholder={'Enter your password'} maxLength={30} placeholderTextColor={'white'}
			onChangeText={(text) => this.setState({password: text})}
			secureTextEntry editable={this.checkPassword()}/>
		<View style={{flexDirection: 'row', paddingLeft: 5}}>
			<Text style={{paddingRight: 20}}>Keep Me Logged In!</Text>
			<Switch value={this.state.autoLogin} onValueChange={() => this.setState({autoLogin: !this.state.autoLogin})}/>
		</View>					
					<Button title="Go to Home" disabled={this.checkButton()} onPress={() => {
			...
			return(
				...
				<View style={{flexDirection: 'row', paddingLeft: 5}}>
					<Text style={{paddingRight: 20}}>Would You Like To have a Password</Text>
					<Switch value={this.state.usePassword} onValueChange={() => {this.setState({usePassword: !this.state.usePassword}); if(this.state.usePassword === false){this.setState({password: ''})}}}/>
				</View>
				<TextInput style = {styles.firstPageInput} value={this.state.password} autoCapitalize='words'
					placeholder={'Enter your password'} maxLength={30} placeholderTextColor={'white'}
					onChangeText={(text) => this.setState({password: text})}
				...
				<Button title="Save Changes and Go to Home" disabled={this.checkButton()}
					onPress={() => {
						this.dispatchUsername()
					}} />
				<Text>                                        </Text>
				<Button title="Go Back" disabled={this.checkButton()}
					onPress={() => {
		...
		else if (this.state.username !== undefined && this.state.shouldUsePassword && !this.state.shouldAutoLogin){
			return(
				<View style={styles.frontPageView}>
					<Text style={styles.smalltexting}>{`Hello ${this.state.username}`}</Text>
					<TextInput style = {styles.firstPageInput} value={this.state.password} autoCapitalize='words'
					placeholder={'Please Enter Your Password'} maxLength={30} placeholderTextColor={'white'}
					secureTextEntry onChangeText={(text) => this.setState({password: text})}/>
						<Button title="Validate and Go to Home" disabled={this.passwordButtonValidate()} onPress={() => {
								this.validatePassword()
							}
						} />
						<Text style={{color: 'red', backgroundColor: 'black', alignSelf: 'center'}}>{this.state.errMessage}</Text>
					</View>
				)			
		}
		else {return(
